...
abstract class Database_Driver {
	...
	{
		throw new Database_Exception(sprintf('The method you called, %s, is not supported by this driver.', __FUNCTION__));
	}
	...
	protected function sql_type($str)
	{
		...
		// DOUBLE
		$sql_types['double'] = $sql_types['double unsigned'] = $sql_types['decimal'] = $sql_types['real'] = $sql_types['numeric'] = $sql_types['float'];		
		// BIT
		$sql_types['bit'] = $sql_types['boolean'];		
		// TIMESTAMP
		$sql_types['timestamp'] = $sql_types['datetime'];		
		// ENUM
		$sql_types['enum'] = $sql_types['set'] = $sql_types['varchar'];		
		// TEXT
...
abstract class Database_Result implements ArrayAccess, Iterator, Countable {
	...
	// celkovy pocet bez ohledu na strankovani @see SQL_CALC_FOUND_ROWS
	...
	{
		throw new Database_Exception('Query results are read only.');
	}
