 ...
 * @package    ORM
 * @author	   Kohana Team
 * @copyright  (c) 2007-2008 Kohana Team, upravy, rozsireni: Ondrej Novak
 * @license    http://kohanaphp.com/license.html
 */
class ORM implements Iterator, IFormData {
	...
	// Current object
	protected $old	= array();
	protected $changed_from_load  = array(); // zmeny od nacteni, narozdil od $changed si data pamatuje i po save()
	...
	// Callback pro nastaveni hodnoty
	protected $callbackSet = array();
	 // Callback pro prevedeni hodnoty
	protected $callbackGet = array();

	// Iterator pointer
	private $position;

	// prenaseni informace, jestli se bude volat onUpdate po save()
	private $callOnUpdate = FALSE;
	// prenaseni informace, jestli se bude volat oninsert po save()
	private $callOnInsert = FALSE;
	...
	public function __initialize()
	{
		if ( ! is_object($this->db))
		{
			...
			$this->db = clone Database::instance($this->db);
	...
	public function __call($method, array $args)
	{
		if (method_exists($this->db, $method))
		{
			...
			{
				throw new BadMethodCallException('Query methods cannot be used through ORM');
			}
			...
			else
			{
				...
				switch ($num_args)
				{
					case 0:
						...
						if($method == 'calc_found_rows') $this->db->$method();
						else return $this->db->$method();
					break;
					case 1:
	...
	public function __get($column)
	{
		if (array_key_exists($column, $this->object))
		{
			...
			// Callback pro prevedeni hodnoty
			if(isset($this->callbackGet[$column])) {
				$value = call_user_func($this->callbackGet[$column], $value);
			}
			return $value;
		}
		...
		// 11.6.2009 pridalo jeste strtolower
		elseif (isset($this->related[strtolower($column)]))
		{
			return $this->related[strtolower($column)];
		}
		...
		elseif (in_array($column, $this->has_many))
		{
			...
			$this->related[$column] = ORM::factory(inflector::singular($column))
		...
		elseif (in_array(strtolower($column), $this->has_and_belongs_to_many))
		{
			...
			if ($this->has($model, TRUE))
			{
				...
				$this->related[ucfirst($column)] = $model
					->in($model->table_name.'.'.$model->primary_key, $this->changed_relations[strtolower($column)])
					->find_all();
				$this->db->reset_select();
				return $this->related[ucfirst($column)];
			}
			else
			{
		...
		{
			throw new /*::*/MemberAccessException("Call to undefined property ".get_class($this).'::$'."$column.");

			//throw new MemberAccessException('core.invalid_property: '.$column.' class:'.get_class($this));
		}
	}
	...
	public function __set($column, $value)
	{
		...
		elseif (isset($this->object[$column]) OR array_key_exists($column, $this->object))
		{
			...
			// Callback pro nastaveni hodnoty
			if(isset($this->callbackSet[$column]) && $value !== NULL) {
				$value = call_user_func($this->callbackSet[$column], $value);
			}

			if (isset($this->table_columns[$column]))
			{
				if($this->object[$column] != $value ||
					(is_string($this->object[$column]) && $this->object[$column] !== $value) ||
					($value === "" && $this->object[$column] !== NULL && $this->object[$column] !== "" && (int)$this->object[$column] === 0) ||
					($value === 0 && $this->object[$column] === NULL) ||
					($value === 0.0 && $this->object[$column] === NULL) ||
					($value === NULL && $this->object[$column] === 0) ||
					($value === NULL && $this->object[$column] === 0.0)
					)
				{
					// Data has changed
					$this->changed[$column] = $column;
					$this->changed_from_load[$column] = $this->object[$column];
	...
	{
		unset($this->object[$column], $this->changed[$column], $this->changed_from_load[$column], $this->related[$column]);
	}
	...
	/**
	* Puvodni objekt pred zmenou
	*
	*/
	public function old()
	{
	   if(!($this->old instanceof ORM)) {
		   $model = ucfirst($this->object_name);
		   $this->old = self::factory($model, $this->old);
	   }
	   return $this->old;
	}
	...
	public function with($target_path)
	{
		...
		$this->db->join($target->db->table_prefix().$target->table_name.' AS '.$this->db->table_prefix().$target_path, $join_col1, $join_col2, 'LEFT');
	 ...
	 * @return	ORM_Iterator
	 */
	public function find_all($limit = NULL, $offset = NULL, $sql_calc_found_rows = true)
	{
		if($sql_calc_found_rows && $this->db->calc_found_rows === null) $this->calc_found_rows(true);
	...
	public function validate(Validation $array, $save = FALSE)
	{
		if ( ! $array->submitted())
		{
			...
			foreach ($safe_array as $key => $value)
			{
				...
				if (is_object($value) AND $value instanceof ORM_List)
				{
	...
	{
		$this->_save();
		if($this->callOnUpdate) $this->onUpdate($this);
		if($this->callOnInsert) $this->onInsert($this);
		$this->onSave($this);
		return TRUE;
	}
	...
	public function _save()
	{
		...
		$this->callOnInsert = FALSE;

		// je zmena nebo je jde o novy insert
		if (!empty($this->changed) || $this->loaded === FALSE)
		{
			...
			if(array_key_exists('updated', $this->object)) {
				$data['updated'] = date('Y-m-d H:i:s');
				$this->object['updated'] = $data['updated'];
			}
			if ($this->loaded === TRUE)
			{
				$query = $this->db
				...
				if(!$query->isOk()) return FALSE; // SQL chyba
			...
			else
			{
				...
				if(array_key_exists('created', $this->object) && empty($data['created'])) {
	...
	public function delete($id = NULL)
	{
		...
		// Delete this object
		$res = $this->db->where($this->primary_key, $id)->delete($this->table_name);
		$this->onDelete($this);
		$this->clear();
		return $res->isOk();
	}
	...
	public function clear()
	{
		...
		// defaultni db hodnoty (Novak)
		foreach($columns as $column) {
			$dbcolumn = $this->table_columns[$column];
			if(isset($dbcolumn['default']) && $dbcolumn['default'] != 'CURRENT_TIMESTAMP') {
					$this->$column = $dbcolumn['default'];
			}
		}
	...
	public function reload_columns($force = FALSE)
	{
		...
		// nastaveni default filtru pro date, datetime a timestamp
		foreach($this->table_columns as $key => $column) {
			if(isset($column['format']) &&
			  ($column['format'] == '0000-00-00 00:00:00' || $column['format'] == '0000-00-00'))  {
				$this->callbackGet[$key] = 'Timestamp::factory';
				if($column['format'] == '0000-00-00') $this->callbackSet[$key] = 'Timestamp::date2Iso';
				else $this->callbackSet[$key] = 'Timestamp::datetime2Iso';
			}
		}
	...
	{
		return $this->object[$this->primary_key] = $value;
	}

	/** Pokus o kopirovani / duplicitu **/
	public function copy()
	{
		$this->changed = array_keys($this->object);
		// unset primarniho klice
		unset($this->changed[array_search($this->primary_key, $this->changed)]);
		$this->object[$this->primary_key] = NULL;
		$this->loaded = false;
	}
	...
	public function foreign_key($table = NULL, $prefix_table = NULL)
	{
		...
		{
			if ( ! is_string($table) OR ! array_key_exists($table, $this->object))
			{
			...
			// Originalni implemenace pocita jako cizi klice pojmenovani parent_id
	...
	protected function related_object($object)
	{
		...
		elseif (isset($this->has_one[strtolower($object)]))
		{
			$object = ORM::factory($this->has_one[strtolower($object)]);
		}
		elseif (isset($this->belongs_to[$object]))
		{
			$object = ORM::factory($this->belongs_to[$object]);
		}
		elseif (isset($this->belongs_to[strtolower($object)]))
		{
			$object = ORM::factory($this->belongs_to[strtolower($object)]);
		}
		elseif (in_array($object, $this->has_one) OR in_array($object, $this->belongs_to))
		{
			$object = ORM::factory($object);
		}
		elseif (in_array(strtolower($object), $this->has_one) OR in_array(strtolower($object), $this->belongs_to))
		{
			$object = ORM::factory($object);
		}
		else
	...
	public function load_values(array $values)
	{
		if (array_key_exists($this->primary_key, $values))
		{
			...
			$this->object = $this->changed = $this->changed_from_load = $this->related = array();
		...
		if (!empty($related))
		{
			foreach ($related as $object => $values)
			{
				...
				// (Novak) pridan pristup k objektu pres velke pismeno
				$this->object[ucfirst($object)] = $this->related[$object];
			}
		}
		return $this;
	}
	...
	protected function load_type($column, $value)
	{
		...
		// jestli je v db default null a hodnota je prazdny retezec, tak vrati NULL
		if ($value === '' AND !empty($column['null'])) return NULL;

		switch ($column['type'])
		{
			case 'int':
				// Novak - nacteni default hodnoty
				if(!isset($column['null']) && $value === NULL && isset($column['default'])) {
	...
	protected function load_result($array = FALSE)
	{
		...
		if ($array === TRUE)
		{
			...
			return new ORM_List($this, $result);
		}
	...
	{
		return (object) $this->changed_from_load;
	}

	/**
	* Test jeztli se zmenil nejaky atribut.
	* V pripade, ze $key = NULL se testuje obecne,
	* jestli nastala zmena od nacteni.
	*
	* @param mixed $key
	* @return bool
	*/
	public function isChanged($key = NULL) {
		...
		if($key == NULL) return !empty($this->changed_from_load);
		return array_key_exists($key, $this->changed_from_load);
	}
	...
	public function setData($data)
	{
		foreach ($data as $key => $value) {
			try {
				...
				// ak je v klici ->, pokusi se hodnotu nastavit na navazanych objektech
				$chainPieces = explode(self::MEMBER_SEPARATOR, $key);
				$tail = array_pop($chainPieces);
				$currentHead = $this;
				foreach ($chainPieces as $memberName) {
					$currentHead = $currentHead->__get($memberName);
				}
				$currentHead->$tail = $value;
			}
		}
	}
	...
	function current() {
		...
		return $this->object[$keys[$this->position]];
	}
