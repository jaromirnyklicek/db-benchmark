...
abstract class Presenter extends Control implements IPresenter
{
    ...
    public $oldDirectoryStructure = FALSE;
	...
	public function formatTemplateFiles($presenter, $view)
	{
        ...
        if($this->oldDirectoryStructure) {
            $root = Environment::getVariable('appDir').'/templates';
...
//					if ((string) $val !== (isset($tmp[$key]) ? (string) $tmp[$key] : '')) {
	...
	private static function argsToParams($class, $method, & $args, $supplemental = array())
	{
		...
		foreach ($params as $name => $def) {
			...
			if ($def === NULL) {
				if ($args[$name] === $def || ($def === NULL && is_scalar($args[$name]) && (string) $args[$name] === '')) $args[$name] = NULL; // value transmit is unnecessary
			} else {
