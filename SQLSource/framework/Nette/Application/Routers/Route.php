...
class Route extends Object implements IRouter
{
	...
	public function match(IHttpRequest $httpRequest)
	{
		...
		foreach ($matches as $k => $v) {
			if (is_string($k) && $v !== '') {
				$this->params[str_replace('___', '-', $k)] = $v; // trick
			}
		}
		...
		foreach ($this->metadata as $name => $meta) {
			if (isset($this->params[$name])) {
				...
				$this->params[$name] = NULL; // cannot be overwriten in 3) and detected by isset() in 4)
			}
		}
		...
		// 3) QUERY
		if ($this->xlat) {
			$this->params += self::renameKeys($httpRequest->getQuery(), array_flip($this->xlat));
		} else {
			$this->params += $httpRequest->getQuery();
		}
		...
		foreach ($this->metadata as $name => $meta) {
			if (isset($this->params[$name])) {
				...
				} elseif (isset($meta[self::FILTER_TABLE][$this->params[$name]])) { // applyies filterTable only to scalar parameters
					$this->params[$name] = $meta[self::FILTER_TABLE][$this->params[$name]];

				} elseif (isset($meta[self::FILTER_IN])) { // applyies filterIn only to scalar parameters
					$this->params[$name] = call_user_func($meta[self::FILTER_IN], (string) $this->params[$name]);
					if ($this->params[$name] === NULL && !isset($meta['fixity'])) {
						return NULL; // rejected by filter
					}
				}

			} elseif (isset($meta['fixity'])) {
				$this->params[$name] = $meta[self::VALUE];
			}
		}


		// 5) BUILD PresenterRequest
		if (!isset($this->params[self::PRESENTER_KEY])) {
			throw new InvalidStateException('Missing presenter in route definition.');
		}
		if (isset($this->metadata[self::MODULE_KEY])) {
			if (!isset($this->params[self::MODULE_KEY])) {
				throw new InvalidStateException('Missing module in route definition.');
			}
			$presenter = $this->params[self::MODULE_KEY] . ':' . $this->params[self::PRESENTER_KEY];
			unset($this->params[self::MODULE_KEY], $this->params[self::PRESENTER_KEY]);

		} else {
			$presenter = $this->params[self::PRESENTER_KEY];
			unset($this->params[self::PRESENTER_KEY]);
		}
