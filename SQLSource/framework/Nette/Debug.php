...
final class Debug
{
		...
		if (is_bool($var)) {
			...
			if (empty($var)) {
				...
				foreach ($var as $k => &$v) {
					...
					if (is_string($k) && in_array($k, array('password', 'pass', 'passwd', 'heslo', 'hovnokleslo'))) {
			...
			if (empty($arr)) {
				...
				foreach ($arr as $k => &$v) {
					...
					if (!in_array($k, array('password', 'pass', 'passwd', 'heslo'))) {
	 ...
	 * @return void
	 */
	public static function processException(Exception $exception, $outputAllowed = FALSE, $processFatal = TRUE)
	{
		if (!self::$enabled) {
			return;
			...
			try {
				...
				$e = "PHP Fatal error: Uncaught " . str_replace("Stack trace:\n" . $exception->getTraceAsString(), '', $exception);
				self::log($e);
				// pokud jiz log existuje, tak se negeneruje znova
				// zakomentovano, generuje se vzdy
...
//				}
				...
				if (self::$sendEmails) {
					self::sendEmail((string) $exception, $hash);
				}
				$e = "PHP Fatal error: Uncaught " . str_replace("Stack trace:\n" . $exception->getTraceAsString(), '', $exception);
				Log::write(MessageLogger::ERROR, $e, array('file' => empty($skip) ? $file : NULL));
			} catch (/*\*/Exception $e) {
				if (!headers_sent()) {
					header('HTTP/1.1 500 Internal Server Error');
				}
	 ...
	 * @return void
	 */
	private static function sendEmail($message, $hash)
	{
		$monitorFile = self::$logFile . $hash . '.monitor';
