...
class LatteMacros extends Object
{
	...
	public static $defaultMacros = array(
		...
		'adminsnippet' => '<?php %:macroSnippet2% ?>',
		'/adminsnippet' => '<?php %:macroSnippet2End% ?>',
	 ...
	 * {snippet ...}
	 */
	public function macroSnippet2($content)
	{
		if (substr($content, 0, 1) === ':' || !$this->oldSnippetMode) { // experimental behaviour
			$this->oldSnippetMode = FALSE;
			return $this->macroBlock('_' . ltrim($content, ':'), '');
		...
		if ($content) {
			$args[] = $this->formatLink($content);
		}
		return '} if ($_cb->foo = SnippetHelper::create2($control' . implode(', ', $args) . ')) { $_cb->snippets[] = $_cb->foo';
	}
	 ...
	 * {snippet ...}
	 */
	public function macroSnippet2End($content)
	{
		if (!$this->oldSnippetMode) {
			return $this->macroBlockEnd('', '');
		}
		return 'array_pop($_cb->snippets)->finish2(); } if (SnippetHelper::$outputAllowed) {';
	}
