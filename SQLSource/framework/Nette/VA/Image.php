...
class Image extends Object
{
	 ...
	 * @return void
	 */
	public function resize($newWidth, $newHeight, $flags = 0, $crop = Thumb::CROP_NONE)
	{
		if(!$crop) list($newWidth, $newHeight) = self::calculateSize($this->getWidth(), $this->getHeight(), $newWidth, $newHeight, $flags);
        ...
        if($crop) {
	        ...
	        if ($ratio[0] < $ratio[1]) { // crop x	        
	        		$dif = $ratio[1] / $ratio[0];
	                $height = $this->getHeight() / $dif;	
	                switch ($crop)
					{
						case 2:
						case 3: $y = 0;	break;
						case 1: $y = ($this->getHeight() - $height) / 2; break;
						case 4: 
						case 5: $y = $this->getHeight() - $height;	break;
					}	
	                
	        } else {
	        		$dif = $ratio[0] / $ratio[1];
	        		$width = $this->getWidth() / $dif; 
	        		switch ($crop)
					{
	        			case 2:
						case 4: $x = 0;
								break;
						case 1: $x = ($this->getWidth() - $width) / 2; break;
						case 3: 
						case 5: $x = $this->getWidth() - $width; break;
					}
	        }
		}
		
		imagecopyresampled($newImage, $this->getImageResource(), 0, 0, $x, $y, $newWidth, $newHeight, $width, $height);     
        $this->image = $newImage;
	}
     ...
     * @return void
     */
    public function resizeAlpha($newWidth, $newHeight, $flags = 0, $crop = Thumb::CROP_NONE)
    {   
    	if(!$crop) list($newWidth, $newHeight) = self::calculateSize($this->getWidth(), $this->getHeight(), $newWidth, $newHeight, $flags);
        if($newWidth == $this->getWidth() && $newHeight == $this->getHeight()) return;
        ...
        /* pokus o pruhlednost gifu - nefunguje mi to :(
	...
	{
		imagefilter($this->getImageResource(), IMG_FILTER_GRAYSCALE);
		return $this;
	}
	...
	{
		imagefilter($this->getImageResource(), IMG_FILTER_NEGATE);
		return $this;
	}
	...
	public function place(Image $image, $left = 0, $top = 0, $opacity = 100)
	{
		...
		if ($opacity <> 0) $this->filter_opacity($image->getImageResource(), $opacity);
        imagecopy($this->getImageResource(), $image->getImageResource(), $left, $top, 0, 0, $image->getWidth(), $image->getHeight());
	}

    protected function filter_opacity($img, $opacity = 100)
    {
        ...
        // zjisti sirku a vysku zadaneho obrazku
        $w = imagesx($img);
        $h = imagesy($img);
        ...
        // najde nejvice nepruhledny bod (s nejmensi hodnotou alfy)
        $minalpha = 127;
        for($x = 0; $x < $w; $x++)
            for($y = 0; $y < $h; $y++)
                {
        ...
        // prchazi skrz vsechny pixely obrazku a pro kazdy nastavi alfu
        for($x = 0; $x < $w; $x++){
            for($y = 0; $y < $h; $y++){
                    // vezme hodnotu alfy aktualniho pixelu
                    ...
                    //nastavi pixel s novou barvou a sytosti
