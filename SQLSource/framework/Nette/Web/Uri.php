...
class Uri extends FreezableObject
{
	...
	public function __construct($uri = NULL)
	{
		if (is_string($uri)) {
			...
			/**
			 * Doporučená hlavička pro http proxy je X-Forwarded-Proto pro popis hlavičky @see http://en.wikipedia.org/wiki/List_of_HTTP_header_fields
			 */
			if (array_key_exists('X-Forwarded-Proto', $_SERVER)) {
				$this->scheme = $_SERVER['X-Forwarded-Proto'];
			}

			/**
			 * Tato hlavička má stejný význam jako X-Forwarded-Proto ale větši prioritu. Je zde pro BC s Sfinance.cz
			 * Obecné doporučení je tuto hlavičku nepoužívat. a namísto ní použít X-Forwarded-Proto
			 */
			if (array_key_exists('HTTP_VA_PROTO', $_SERVER)) {
				$this->scheme = $_SERVER['HTTP_VA_PROTO'];
			}
