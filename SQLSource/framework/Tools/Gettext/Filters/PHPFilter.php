	...
	public function extract($file)
	{
		...
		{			
			if(is_array($c)) {
				...
				if ($c[0] == T_STRING && isset($this->functionsP[$c[1]])) {
					$nextP = 1;
					continue;
				}
				...
				if ($c[0] == T_CONSTANT_ENCAPSED_STRING && ($nextP == 1 || $nextP == 2)) {
					if($nextP == 1) $str1 = substr($c[1], 1, -1);
					else {
						$data[$str1.'\00'.substr($c[1], 1, -1)][] = $pInfo['basename'] . ':' . $c[2];
						$nextP = false; 
					}					 
				}
			} else {
				if ($c == ')') { $next = false; $nextP = false; }
