...
class GettextExtractor
{
	...
	protected function formatData($data)
	{
		...
		foreach ($data as $key => $files)
		{
			...
			$output[] = '# ' . $file;			 
			if (preg_match("#\\\\00#", $key, $matches)) {	  
				$arr = explode('\\00', $key);
				$output[] = 'msgid "' . $this->addSlashes($arr[0]) . '"';
				$output[] = 'msgid_plural "' . $this->addSlashes($arr[1]) . '"';
				$output[] = 'msgstr[0] "' . $this->addSlashes($arr[0]) . '"';
				$output[] = 'msgstr[1] "' . $this->addSlashes($arr[1]) . '"';
			} else {
